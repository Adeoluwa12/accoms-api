...
/**
 * SCORING ALGORITHM
 * fellowship = unified church centre / fellowship field.
 * Counts how many occupants in a unit share the same fellowship as the
 * incoming attendee. Higher count = worse fit for distribution.
 */
async function scoreUnit(unit, fellowship) {
  if (!fellowship) return 0;
  const count = await Attendee.countDocuments({
    accommodationId: unit._id,
    fellowship: { $regex: new RegExp(`^${fellowship.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}$`, 'i') },
  });
  return count;
}
 ...
 * Converts scores into inverse weights so that units with lower
 * concentration of the same fellowship get picked more often — but
 * any unit can still be chosen, preventing chronological fill-up.
 *
 * Weight formula:  w = 1 / (score + 1)
 * Units with score 0  → weight 1.0   (full chance)
 * Units with score 1  → weight 0.5   (half chance)
 * Units with score 3  → weight 0.25  (quarter chance)
 *
 * A random number in [0, totalWeight) selects the winner.
 */
function weightedRandom(scored) {
  const weighted = scored.map(({ unit, score }) => ({
    unit,
    weight: 1 / (score + 1),
  }));
 ...
 * AUTO-ASSIGN
 * Rooms first → Dorms second.
 * Uses weighted random so rooms fill in a shuffled, distributed manner
 * rather than filling one room completely before moving to the next.
 * Reserved slots are invisible to auto-assignment.
 * If all units are full → return NO_CAPACITY error (admin must add capacity).
 */
async function autoAssign(attendee) {
  const { gender, fellowship, eventId } = attendee;

  const candidates = await Unit.find({ eventId, gender, isActive: true });
  ...
  // Score each unit in pool for fellowship concentration
  const scored = await Promise.all(
    pool.map(async (unit) => {
      const score = await scoreUnit(unit, fellowship);
      return { unit, score };
    })
  );

  // Pick via weighted random — distributes without chronological fill-up
  const bestUnit = weightedRandom(scored);
  return { success: true, unit: bestUnit };
}
